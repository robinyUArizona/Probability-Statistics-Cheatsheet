...
\vspace{2pt}
\textcolor{purple}{Exponential} - memoryless time  between independent event occuring at an average rate $\boxed{\lambda \rightarrow \lambda e^{\lambda x}}$, with $\boxed{\mu = \frac{1}{\lambda}}$

\vspace{2pt}

\textcolor{purple}{Gamma} -  time until $n$ independent events occuring at an average rate $\lambda$.

\begin{center}
    \begin{minipage}{\linewidth}
    \includegraphics[width=\textwidth]{figures/Probability_Distribution.PNG}
    \end{minipage}
\end{center}
...
$\bullet$ \textbf{\textcolor{purple}{Where is the variable centered?}} called \textit{\textbf{measure of central of tendency}}. $\rightarrow$ mean, median, mode\\
 
$\rightarrow$ Outliers draw the \textbf{mean} towards them! (also effect SD), critical for computation and prediction \\
...
$\rightarrow$ \textbf{Variance} (mean square distance from the mean) and SD\\
...
\begin{center}
    ...
    \includegraphics[width=\textwidth]{figures/Hypothesis_Testing_Steps.PNG}
    \end{minipage}
\end{center}
...
\begin{center}
    \begin{minipage}{\linewidth}
    \includegraphics[width=\textwidth]{figures/p_value_and_critical_value.PNG}
    \end{minipage}
\end{center}
...
$\rightarrow$ An A/B test is an experiment with two groups to establish which of two treatments, products, procedures, or the like is superior. Two groups:\\
$\rightarrow$ \textbf{Treatment group}: A group of subjects exposed to a specific treatment. \\
$\rightarrow$ \textbf{Control group}: A group of subjects exposed to no (or standard) treatment.\\
\textbf{Ex}: Testing two therapies to determine which suppresses cancer more effectively \\
...
$\bullet$ \textbf{z-Test}: Generally, z-Test is used when the \textbf{sample size} $n$ is \textbf{large} $n \geq 30$ (to invoke the CLT) and when the population \textbf{variance} (or standard deviation) is \textbf{known}.\\

$\rightarrow$ \textbf{One-Sample} z-Test: compare a \textbf{sample mean} $\bar{x}$ with the \textbf{population mean} $\mu$
\vspace{-5pt}
\[\boxed{z = \frac{\bar{x} - \mu_{0}}{\frac{\sigma}{\sqrt{n}}} \sim N(0,1)} \]

$\rightarrow$ \textbf{Two-Sample} z-Test: compare the \textbf{mean} of two samples
\vspace{-5pt}
\[\boxed{z = \frac{\left(\bar{x_{1}} - \bar{x_{2}} \right) - \left( \mu_{1} - \mu_{2} \right)}{\sqrt{\frac{\sigma^{2}_{1}}{n_{1}} + \frac{\sigma^{2}_{2}}{n_{2}}}}}\]
